                ...
                for p1 in plist:
                    if p1["label"] != bait:
                        if  p1["label"] > bait:
                            ...
                            key = bait+ '(pp)' + p1["label"]
                        else:
                            #print( p1["label"], 'pp', bait )
                            key =  p1["label"] + ' (pp) ' + bait

                        if not key in exptype:
                            exptype[key] = [ ]
                        exptype[key].append( "affinity" )
                ...
                for p1 in plist:
                    for p2 in plist:
                        if p1["label"] <  p2["label"]:
                            ...
                            key =  p1["label"] + ' (pp) ' +  p2["label"]
                         
                            if not key in exptype:
                                exptype[key] = [ ]
                            exptype[key].append("physical")
                          

            if i11n["itype"]["label"] == "direct":
                for p1 in plist:
                    for p2 in plist:
                        if p1["label"] <  p2["label"]:
                            #print( p1["label"], 'pp',  p2["label"] )
                            ...
                            if not key in exptype:
                                exptype[key] = [ ]
                            exptype[key].append("direct")

print( "key count: ",len(list(exptype.items())))    
print("---- property table starts here ----")

for k, v in exptype.items():
